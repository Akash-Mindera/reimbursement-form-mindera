...
const ApprovedRecord = (props) => {
  ...
  }, [user, props.tab2, props.employeeID]);

  const getResponse = async () => {
    if (props.employeeID) {
      const response = await axios.get(
        `/approvedUserData/${props.employeeID}?page=0`,
        {
          headers: {
      ...
      setApprovedUserData(response.data.data);
  ...
  const handlePageClick = async (data) => {
    ...
    const response = await axios.get(
      `/approvedUserData/${props.employeeID}?page=${currentPage}`,
      {
        headers: {
          authToken: user.accessToken,
        },
