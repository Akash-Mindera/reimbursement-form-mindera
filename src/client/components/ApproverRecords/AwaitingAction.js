...
const AwaitingAction = (props) => {
  ...
  const [pendingApproverData, setPendingApproverData] = useState([]);
  ...
  const getResponse = async () => {
    if (props.employeeID) {
      const response = await axios.get(
        `/pendingApproverData/${props.employeeID}?page=0`,
        {
          headers: {
      ...
      setPendingApproverData(response.data.data);
  ...
  const handlePageClick = async (data) => {
    ...
    const response = await axios.get(
      `/pendingApproverData/${props.employeeID}?page=${currentPage}`,
      {
        headers: {
          authToken: user.accessToken,
        },
