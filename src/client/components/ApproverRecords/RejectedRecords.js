...
const RejectedRecord = (props) => {
  ...
  const [rejectedApproverData, setRejectedApproverData] = useState([]);
  ...
  const getResponse = async () => {
    if (props.employeeID) {
      const response = await axios.get(
        `/rejectedApproverData/${props.employeeID}?page=0`,
        {
          headers: {
      ...
      setRejectedApproverData(response.data.data);
  ...
  const handlePageClick = async (data) => {
    ...
    const response = await axios.get(
      `/rejectedApproverData/${props.employeeID}?page=${currentPage}`,
      {
        headers: {
          authToken: user.accessToken,
        },
