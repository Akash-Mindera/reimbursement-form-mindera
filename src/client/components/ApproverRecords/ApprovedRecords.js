...
const ApprovedRecord = (props) => {
  ...
  const [approvedApproverData, setApprovedApproverData] = useState([]);
  ...
  const getResponse = async () => {
    if (props.employeeID) {
      const response = await axios.get(
        `/approvedApproverData/${props.employeeID}?page=0`,
        {
          headers: {
      ...
      setApprovedApproverData(response.data.data);
  ...
  const handlePageClick = async (data) => {
    ...
    const response = await axios.get(
      `/approvedApproverData/${props.employeeID}?page=${currentPage}`,
      {
        headers: {
          authToken: user.accessToken,
        },
