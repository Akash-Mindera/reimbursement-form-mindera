...
const AwaitingAction = (props) => {
  ...
  const getResponse = async () => {
    if (props.employeeID) {
      const response = await axios.get(
        `/pendingAdminData/${props.employeeID}?page=0`,
        {
          headers: {
      ...
      setPendingAdminData(response.data.data);
  ...
  const handlePageClick = async (data) => {
    ...
    const response = await axios.get(
      `/pendingAdminData/${props.employeeID}?page=${currentPage}`,
      {
        headers: {
          authToken: user.accessToken,
        },
