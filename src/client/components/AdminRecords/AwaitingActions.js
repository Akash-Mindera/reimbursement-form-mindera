...
const AwaitingAction = (props) => {
  ...
  const filteredAwaitedRecords = props.adminRemRecords.filter(
    (id) =>
      props.adminData[id].IsApproved === "" &&
      props.adminData[id].UserSpecificId === props.employeeID
  );

  const adminAwaitedRecords = filteredAwaitedRecords.slice(
    indexOfFirstPost,
    indexOfLastPost
  );
