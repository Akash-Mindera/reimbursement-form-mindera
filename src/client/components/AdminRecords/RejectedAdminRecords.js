...
const RejectedAdminRecord = (props) => {
  ...
  const [rejectedAdminData, setRejectedAdminData] = useState([]);
  ...
  const getResponse = async () => {
    if (props.employeeID) {
      const response = await axios.get(
        `/rejectedAdminData/${props.employeeID}?page=0`,
        {
          headers: {
      ...
      setRejectedAdminData(response.data.data);
  ...
  const handlePageClick = async (data) => {
    ...
    const response = await axios.get(
      `/rejectedAdminData/${props.employeeID}?page=${currentPage}`,
      {
        headers: {
          authToken: user.accessToken,
        },
