...
const RequiresApprovalFromAdmin = (props) => {
  return (
    <Card>
      <div className="remList-single" style={{ marginBottom: "10px" }}>
        <h3 className="h3Field-class">
          ...
          <span className="spanDta-field">
            {props.filterPendingAdminDataUsingId.length}
          </span>
        </h3>
      </div>
      {props.filterPendingAdminDataUsingId.length !== 0 && (
      ...
      <div className="div-horizontal">
        ...
        >
          ...
          <tbody>
            {props.filterPendingAdminDataUsingId.map((id) => (
              <tr key={id}>
                <td>{id}</td>
...
export default RequiresApprovalFromAdmin;
