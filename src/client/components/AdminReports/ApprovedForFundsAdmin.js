...
const ApprovedForFundsAdmin = (props) => {
  return (
    <Card>
      <div className="remList-single" style={{ marginBottom: "10px" }}>
        <h3 className="h3Field-class">
          ...
          <span className="spanDta-field">
            {props.filterProcessedAdminDataUsingId.length}
          </span>
        </h3>
      </div>
      {props.filterProcessedAdminDataUsingId.length !== 0 && (
      ...
      <div className="div-horizontal">
        ...
        >
          ...
          <tbody>
            {props.filterProcessedAdminDataUsingId.map((id) => (
              <tr key={id}>
                <td>{id}</td>
...
export default ApprovedForFundsAdmin;
