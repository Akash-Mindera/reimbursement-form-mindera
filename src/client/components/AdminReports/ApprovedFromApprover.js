...
const ApprovedFromApprover = (props) => {
  return (
    <Card>
      <div className="remList-single" style={{ marginBottom: "10px" }}>
        <h3 className="h3Field-class">
          ...
          <span className="spanDta-field">
            {props.filterApprovedDataUsingId.length}
          </span>
        </h3>
      </div>
      {props.filterApprovedDataUsingId.length !== 0 && (
      ...
      <div className="div-horizontal">
        ...
        >
          ...
          <tbody>
            {props.filterApprovedDataUsingId.map((id) => (
              <tr key={id}>
                <td>{id}</td>
...
export default ApprovedFromApprover;
